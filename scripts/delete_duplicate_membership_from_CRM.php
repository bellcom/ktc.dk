...
while ($record = $result->fetchAssoc()) {
  ...
  if ($webuserdto) {
    ...
    foreach ($duplicates as $crmuid) {
      if (!db_query('SELECT field_crm_uuid_value  FROM {field_data_field_crm_uuid} WHERE field_crm_uuid_value = :id', array(':id' => $crmuid))->fetchField())
        ktc_crm_group_membership_delete($crmuid);
       print 'Deleted membership' .  $crmuid;
    }    
  }
}